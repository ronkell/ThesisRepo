...
import numpy as np
class POMCP():
    def __init__(self, blackbox, discountfactor=0.95, c=1, threshold=0.005, timeout=1000, num_of_patricles=1000):
    ...
    def search(self, h=-1):
        ...
        for i in range(self.timeout):
            if prevBeliefState == []:
                ...
                prevplaster = np.empty(len(prevBeliefState), dtype=object)
                prevplaster[:] = prevBeliefState
                s = choice(prevplaster)
            self.simulate(s, h, 0)
    ...
    def rollout(self, s, h, depth):
        if (self.discountfactor ** depth < self.epsilon or self.discountfactor == 0) and depth != 0:
            return 0
        accumlate_reward = 0
    ...
    def simulate(self, s, h, depth):
        ...
        if self.tree.isLeaf(h):  # if h is  leaf
            for action in self.actions:
                #convertedaction=self.convertToIndex(True,action)
    ...
    def getObservationNode(self, h, sample_obs):
        #converted_obs=self.convertToIndex(False,sample_obs)
    ...
    def PostSample(self, bh, action, observation, d):
        if bh == []:
            ...
            bhplaster = np.empty(len(bh), dtype=object)
            bhplaster[:] = bh
            s = choice(bhplaster)
